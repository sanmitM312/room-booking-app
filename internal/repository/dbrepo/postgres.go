...
func (m *postgresDBRepo) Authenticate(email, testPassword string)(int,string,error){
	...
	if err == bcrypt.ErrMismatchedHashAndPassword{
		return 0, "" , errors.New("incorrect passoword")
	}else if err != nil {
		return 0, "", err
	}
...
func (m *postgresDBRepo) AllNewReservations()([]models.Reservation, error){
	ctx,cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()// AllReservations returns a slice of reservations where processed = 0/ new reservations
...
// AllRooms returns a slice of the rooms 
...
func (m *postgresDBRepo) GetRestrictionsForRoomByDate(roomID int, start, end time.Time)([]models.RoomRestriction,error){
	...
	// reservation ids might be NULL for some room restriction
	// so need to use coalesce for it
	for rows.Next() {
