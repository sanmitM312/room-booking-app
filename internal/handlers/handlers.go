...
func (m *Repository) AdminReservationsCalendar(w http.ResponseWriter, r *http.Request) {
	...
	currentLocation := now.Location() // for UTC
	...
	for _, x := range rooms {
		// create maps to get on which days the room is blocked
		...
		for _, y := range restrictions {
			if y.ReservationID > 0 {
				...
				// it's a block
				// fmt.Printf("Block Date : %s with restriction ID : %d",y.StartDate.Format("2006-01-2"),y.ID)
...
//AdminDeleteRese  rvation  deletes a reservation
...
// AdminPostReservationsCalendar handles post of the data
func (m *Repository) AdminPostReservationsCalendar(w http.ResponseWriter, r *http.Request) {
	...
	// process the incoming information from the blocks
	rooms, err := m.DB.AllRooms()
	if err != nil {
		helpers.ServerError(w, err)
		return
	...
	for _, x := range rooms {
		...
		// Loop through entire map, if we have entry in the map
		// that does not exist in our posted data and if the restriction id > 0
		curMap := m.App.Session.Get(r.Context(), fmt.Sprintf("block_map_%d", x.ID)).(map[string]int)
		for name, value := range curMap {
			//ok will be false if the value is not in the map, like if the block has some value
			if val, ok := curMap[name]; ok {
				// only pay attention to values > 0, i.e restriction id , and that are not in the form post
				// the rest are just placeholders for days without blocks
				if val > 0 {
					// remove_block name checkboxes have the checked blocks
